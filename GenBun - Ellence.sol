...
contract GenBunEllence is Ownable, ERC721Burnable, ERC721Pausable, ERC721Enumerable{
    ...
    constructor(string memory baseUri, address _signer, IERC20 _token) ERC721("GenBun: Ellence", "GENBUN-L"){
        setBaseUri(baseUri);
        pause(true);
        signer = _signer;
        token = _token;
    ...
    function mint(uint256 payment) public{
        require(totalSupply() <= 1778, "GenBun: Quantity of minting exceeds public sale!"); // ensures that minting will not exceed max number of NFT.
